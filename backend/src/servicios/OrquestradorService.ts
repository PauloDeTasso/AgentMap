...
export class OrquestradorService {
  ...
  async dispatch(payload: OrquestradorDispatchPayload): Promise<ResultadoOperacao<any>> {
    return { sucesso: false, erro: 'Endpoint depreciado: dispatch via CLI Kilo não está disponível. Use Agent Manager worktrees.', codigoErro: 'NOT_IMPLEMENTED' };
  }
  ...
  async recuperarEstado(): Promise<ResultadoOperacao<{ daemonsVerificados: number; tarefasOrfas: number; reconciliadas: number }>> {
    return { sucesso: true, dados: { daemonsVerificados: 0, tarefasOrfas: 0, reconciliadas: 0 } };
  }
}
