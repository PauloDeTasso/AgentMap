...
export class InstanciaService {
  ...
  listar(filtros?: { agenteId?: string; projetoId?: string; workspacePath?: string; status?: string }): ResultadoOperacao<Instancia[]> {
    ...
    if (filtros?.agenteId) instancias = instancias.filter((i: Instancia) => i.agenteId === filtros.agenteId);
    if (filtros?.projetoId) instancias = instancias.filter((i: Instancia) => i.projetoId === filtros.projetoId);
    if (filtros?.workspacePath) instancias = instancias.filter((i: Instancia) => i.workspacePath === filtros.workspacePath);
    if (filtros?.status) instancias = instancias.filter((i: Instancia) => i.status === filtros.status);
  ...
  async criar(dados: Partial<Instancia>): Promise<ResultadoOperacao<Instancia>> {
    ...
    if (registryResult.dados.instancias.find((i: Instancia) => i.instanciaId === instanciaId)) {
  ...
  async atualizar(id: string, dados: Partial<Instancia>): Promise<ResultadoOperacao<Instancia>> {
    ...
    const registryResult = this.carregarRegistry();
    if (registryResult.sucesso && registryResult.dados) {
      registryResult.dados.instancias = registryResult.dados.instancias.map((i: Instancia) => (i.id === id ? atualizado : i));
      this.salvarRegistry(registryResult.dados);
    }
  ...
  async excluir(id: string): Promise<ResultadoOperacao<boolean>> {
    ...
    const instancia = registryResult.dados.instancias.find((i: Instancia) => i.id === id);
    if (!instancia) {
      console.warn('[INSTANCIA][EXCLUIR] nao encontrada', JSON.stringify({ id }));
      return { sucesso: false, erro: 'Instância não encontrada', codigoErro: 'NOT_FOUND' };
    }

    registryResult.dados.instancias = registryResult.dados.instancias.filter((i: Instancia) => i.id !== id);
    this.salvarRegistry(registryResult.dados);
