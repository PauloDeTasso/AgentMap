...
registerTracedTool(mcpServer, 'agentmap_criterios_excluir_todos', {
  title: 'Excluir Todos os Critérios',
  description: 'Exclui todas as critérios do projeto.',
  inputSchema: z.object({}),
  ...
  const { projeto } = ctx.dados!;
  const auditoria = createMcpAuditoria(projeto.auditoria);
  const resultado = await ctx.dados!.servicos.criterio.excluirTodos();
  auditoria.registrarToolCall('agentmap_criterios_excluir_todos', projeto, {}, resultado);
  if (!resultado.sucesso) return mcpError(resultado);
  return toMcpStructured(resultado.dados);
});
