...
registerTracedTool(mcpServer, 'agentmap_bloqueios_excluir_todos', {
  title: 'Excluir Todos os Bloqueios',
  description: 'Exclui todas as bloqueios do projeto.',
  inputSchema: z.object({}),
  ...
  const { projeto } = ctx.dados!;
  const auditoria = createMcpAuditoria(projeto.auditoria);
  const resultado = await ctx.dados!.servicos.bloqueio.excluirTodos();
  auditoria.registrarToolCall('agentmap_bloqueios_excluir_todos', projeto, {}, resultado);
  if (!resultado.sucesso) return mcpError(resultado);
  return toMcpStructured(resultado.dados);
});
